...
void save_p_and_cs(const pipe& p, const compressor_station& cs)
{
	...
	if (file.is_open()) {
		...
		file << "Длина трубы: " << p.length << endl;
		file << "Диаметр трубы: " << p.diameter << endl;
		if (p.UnderRepair) {
		...
		file << "Количество цехов:  " << cs.quantity << endl;
		file << "Количество цехов в работе:  " << cs.quantity_cs << endl;
		file << "Эффективность:  " << cs.effective_cs << endl;
	}
}
